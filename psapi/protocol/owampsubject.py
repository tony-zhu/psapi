#!/usr/bin/env python
"""
Specific OWAMP Subject that takes only EndPointPair as a content.
"""
__authors__ = [
...
class OWAMPSubject(Subject):
    ...
    def from_xml(xml):
        ...
        return OWAMPSubject(contents, object_id, ref_id)

    def to_xml(self, parent=None, tostring=True):
        """Serialize to XML representation."""
        if parent is None:
            tree = etree.Element('{%s}subject' % ns.OWAMP, nsmap=ns.nsmap)
        else:
            tree = etree.SubElement(parent, '{%s}subject' % ns.OWAMP)
