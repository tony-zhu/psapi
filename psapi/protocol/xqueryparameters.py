...
class XQueryParameters(Parameters):
    ...
    @staticmethod
    def from_xml(xml):
        tree = PsObject.assert_xml(xml, '{%s}parameters' % ns.XQUERY)
    ...
    def to_xml(self, parent=None, tostring=True):
        ...
        if parent is None:
            tree = etree.Element('{%s}parameters' % ns.XQUERY, nsmap=ns.nsmap)
        else:
            tree = etree.SubElement(parent, '{%s}parameters' % ns.XQUERY)
