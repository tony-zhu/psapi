...
class PingerParameters(Parameters):
    ...
    @staticmethod
    def from_xml(xml):
        tree = PsObject.assert_xml(xml, '{%s}parameters' % ns.PINGER)
    ...
    def to_xml(self, parent=None, tostring=True):
        ...
        if parent is None:
            tree = etree.Element('{%s}parameters' % ns.PINGER, nsmap=ns.nsmap)
        else:
            tree = etree.SubElement(parent, '{%s}parameters' % ns.PINGER)
